...
from SprinklerActionPublish import AWS
...
def main_algorithm(config):
    ...
    for sprinkler in sprinklers:
        ...
        print(f'Upper threshold_humidity: {config["upper_threshold_humidity"]}')
        print(f'Lower threshold_humidity: {config["lower_threshold_humidity"]}')
        # humidity_decision = actual_humidity > config['upper_threshold_humidity']

        # check if air temperature level has reached (as per configuration) then return true
        print(f'Upper threshold_air_temp: {config["upper_threshold_air_temp"]}')
        print(f'Lower threshold_air_temp: {config["lower_threshold_air_temp"]}')
        # air_temp_decision = actual_air_temperature > config['upper_threshold_air_temp']
        ...
        for avg_moisture in sensor_wise_avg_moisture:
            ...
            if sensor_wise_avg_moisture[avg_moisture][0]['value'] > config['threshold_soil_mois']:
                switch_on_count = moisture_switch_on_count + 1
        # mois_sensors_decision = sprinkler["min_devices_to_alarm"] < moisture_switch_on_count

        # check for each sensor, if average soil temperature level (last 5 mins) has reached the configured value and
        # check if enough number of devices reporting to alarm, then return true else false
        temperature_switch_on_count = 0
        for avg_temperature in sensor_wise_avg_temperature:
            if sensor_wise_avg_temperature[avg_temperature][0]['value'] > config['threshold_soil_temp']:
                switch_on_count = temperature_switch_on_count + 1
        # temp_sensors_decision = sprinkler["min_devices_to_alarm"] < temperature_switch_on_count
        ...
        if actual_air_temperature > config['upper_threshold_air_temp'] or \
                actual_humidity > config['upper_threshold_humidity']:
            sprinkler_new_action = 'ON'
        elif actual_air_temperature <= config['lower_threshold_air_temp'] or \
                actual_humidity <= config['lower_threshold_humidity']:
            sprinkler_new_action = 'OFF'
        elif temperature_switch_on_count >= sprinkler["min_devices_to_alarm"] or \
                moisture_switch_on_count >= sprinkler["min_devices_to_alarm"]:
        ...
        print(f'Sprinkler {sprinkler["sprinkler_id"]} last action is: {sprinkler_last_action}; and new decision made '
              f'is: {sprinkler_new_action}')
